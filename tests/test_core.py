...
class SkillagerCoreTests(unittest.TestCase):
    ...
    def test_top_level_help_points_agents_to_agentic_setup_flow(self) -> None:
        ...
        self.assertIn("Never activate or expose unavailable skills", help_text)
        self.assertNotIn("unavailable skills unless the user explicitly asks", help_text)
    ...
    def test_working_skill_has_quiet_session_contract(self) -> None:
        text = render_working_skill("codex")
        self.assertIn("name: skillager-working", text)
        ...
        self.assertIn("Treat `next` as required before using or exposing managed skill bodies", text)
        self.assertIn("pending personal-library draft while unrelated project review waits", text)
        self.assertIn("Treat `exposure_changes` as advisory", text)
        self.assertIn("skillager doctor --agent codex --fix", text)
        self.assertIn("Search when the task enters a new domain", text)
    ...
    def test_working_skill_covers_agent_selection_and_owner_library_work(self) -> None:
        text = render_working_skill("codex")
        ...
        self.assertNotIn("confidence score from 0-100", text)
        self.assertIn("Prefer an existing matching", text)
        self.assertIn("router. Keep using the chosen path", text)
        self.assertIn("skillager expose --tag <tag> --mode router --agent codex --scope project", text)
        self.assertIn("skillager expose <skill-id> --mode stub --agent codex --scope project", text)
        self.assertIn("skillager expose <skill-id> --mode native --agent codex --scope project", text)
        self.assertIn("Prefer no new exposure for one-off", text)
        self.assertIn("Owner-Directed Personal Library", text)
        self.assertIn("skillager library status [<lib-skill>] --json", text)
        self.assertIn("skillager library history <lib-skill> --json", text)
        self.assertNotIn("skillager reconcile", text)
        self.assertNotIn("skillager sync", text)
        self.assertNotIn("skillager where", text)
        self.assertIn("Personal-library ownership never approves changed bytes", text)
        self.assertIn("Do not convert external inventory implicitly", text)
        self.assertIn("inspect the relevant command's `--help`", text)
        self.assertIn("skillager library new <name> --json", text)
        self.assertIn("available skill-authoring workflow", text)
        self.assertIn("second scaffold initializer", text)
        self.assertIn("but not acceptance or an override", text)
        self.assertIn("authorizes that narrow exposure once", text)
        self.assertIn("does not waive the acceptance gate", text)
        self.assertIn("only after the user", text)
        self.assertIn("Never infer acceptance from authorship or ownership", text)
...
"""
    ...
    def test_wheel_bundles_user_docs_and_examples(self) -> None:
        ...
        self.assertIn("skillager/docs/EXPOSURE_PLANS.md", names)
