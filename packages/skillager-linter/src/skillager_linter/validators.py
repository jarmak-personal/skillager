...
CARGO_PACKAGE_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")
...
def _targets(value: Any) -> dict[str, Any]:
    ...
    _check_allowed_keys(data, {"python_packages", "npm_packages", "cargo_packages"}, "targets")
    result: dict[str, Any] = {}
    ...
    if "cargo_packages" in data:
        result["cargo_packages"] = _cargo_package_targets(data["cargo_packages"])
    return result
...
def _cargo_package_targets(packages: Any) -> list[dict[str, Any]]:
    if not isinstance(packages, list) or len(packages) > MAX_TARGET_PACKAGES:
        raise ManifestValidationError(
            "targets.cargo_packages invalid",
            findings=[
                finding(
                    "target_package_invalid",
                    "block",
                    "targets.cargo_packages",
                    f"expected list with at most {MAX_TARGET_PACKAGES} entries",
                )
            ],
        )
    ...
    seen: set[tuple[str, str | None]] = set()
    for index, item in enumerate(packages):
        field = f"targets.cargo_packages[{index}]"
        package = _required_mapping(item, field)
        _check_allowed_keys(package, {"name", "versions"}, field)
        name = package.get("name")
        if not isinstance(name, str) or not name.strip():
            raise ManifestValidationError(
                "package name invalid",
                findings=[finding("target_package_invalid", "block", f"{field}.name", "expected Cargo crate name")],
            )
                ...
                raise ManifestValidationError(
                    ...
                    findings=[
                        finding(
                            ...
                            "block",
                            f"{field}.versions",
                            "expected non-empty Cargo semver requirement string",
                        )
                    ],
                )
            versions = " ".join(raw_spec.split())
...
def canonical_cargo_package_name(name: str) -> str:
    return name.strip().lower()
