...
Conflicts, in-progress operations, unrelated staged files, missing or ambiguous hashes, changed previews, unsafe historical trees, and current symlinks or excluded files fail closed. Acceptance applies the same canonical-tree rule before staging, so an ignored symlink cannot poison append-only history. The public version-2 Skillager content hash uses domain-separated, length-framed canonical paths, bytes, and normalized executable bits; neither file boundaries nor mode markers can be confused with content. A mode-only change therefore revokes acceptance. Older ambiguous hashes are not accepted as aliases and require one-time review under the current identity. No-Git libraries explicitly report history as unavailable without affecting ordinary authored/imported library use.

## Incremental Index And Exposure Drift

Skillager stores an advisory fingerprint with each local discovery index entry. The fingerprint covers the same agent-visible file set as `content_hash`, but uses relative path, byte size, nanosecond modification time, and normalized executable state rather than reading file bodies. Every inventory rebuild still recomputes the exact canonical content hash before resolving approval or availability; scanner and linter results are reused only when that exact hash matches the cached entry. `working` also fully hashes personal-library sources and managed exposure targets before reporting availability or drift, and every exposure write recomputes authoritative hashes. Native exposure requires host-valid frontmatter for every source, then copies into a verified candidate and atomically installs it only if candidate and source still reproduce the accepted hash. Fingerprint equality never authorizes trust, drift classification, body emission, or mutation.

New materialization sidecars record a projection-kind-aware stable identity, exact source/materialized hashes, a full managed-target hash, materialized fingerprint, agent, and scope. The managed-target hash covers every target entry, type, mode, and byte except the self-referential sidecar, including entries excluded from canonical skill identity. A separate domain-separated sidecar hash authenticates the canonical sidecar payload itself; raw formatting or field edits therefore classify the projection as locally edited. Library-owned direct exposures additionally record the stable `source_library_id`; router sidecars record each member’s UUID and skill ID in `member_sources`. Public exposure metadata preserves these qualifiers. Canonical association, currentness and source-dependent router restoration/activation require matching UUID and ID before hashes; foreign or unknown identities are never inferred from the connected library. Target-owned removal remains source-independent. See [exposure source identity](EXPOSURE_PLANS.md#public-source-identity) for legacy and malformed metadata behavior. Duplicated ownership labels are not authoritative and are no longer written. `working --json` compares live current-project targets, sidecar source hashes, and current approved source hashes while revalidating personal-library hashes read-only. A clean target behind accepted source content becomes `source_update`, is excluded from current/exposed inventory, and receives an exact re-expose command. Owned drafts and edits remain advisory and do not enter the external setup gate. The check does not change readiness decisions, trust, indexes, sidecars, or target files. Exposure-blocked legacy decisions take precedence over ordinary local-edit classification. Unreadable, incomplete, or unauthenticated hardened sidecars fail closed. Fully deleted exposure directories remain unknowable without a ledger; Skillager does not infer their existence from global state.
