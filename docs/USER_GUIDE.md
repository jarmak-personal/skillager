...
Install Skillager as a global user tool with `uv tool install skillager` or `pipx install skillager`. It scans the current project's `.venv`, `venv`, `.conda`, project-local active conda environments, and top-level `node_modules` for installed package skills, but ordinary projects do not need Skillager installed inside their own Python or JavaScript environment.
